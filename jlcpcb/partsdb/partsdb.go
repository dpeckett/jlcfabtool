...
func CreateFromCSV(dbPath, csvPath string) error {
	...
	parts, err := csvx.Unmarshal[Part](f)
	if err != nil {
		db.Close()
	...
	if err != nil {
		db.Close()
		return fmt.Errorf("could not insert records: %w", err)
	}
...
//go:embed jlcpcb-parts.db
var partsDBData []byte

// Open opens a PartsDB, if dbPath is empty the embedded database is used.
func Open(dbPath ...string) (*PartsDB, error) {
	var tempDir string
	if len(dbPath) == 0 {
		var err error
		tempDir, err = os.MkdirTemp("", "jlcpcb-partsdb")
		if err != nil {
			return nil, fmt.Errorf("could not create temporary directory: %w", err)
		}

		dbPath = []string{filepath.Join(tempDir, "jlcpcb-parts.db")}
		if err := os.WriteFile(dbPath[0], partsDBData, 0o644); err != nil {
			return nil, fmt.Errorf("could not extract embeded database: %w", err)
		}
	}

	db, err := sqlx.Connect("sqlite3", dbPath[0]+"?_journal_mode=WAL&_synchronous=NORMAL")
	if err != nil {
		return nil, fmt.Errorf("could not open database: %w", err)
	}

	return &PartsDB{db: db, tempDir: tempDir}, nil
}
...
func (pdb *PartsDB) Close() error {
	...
	if err := os.RemoveAll(pdb.tempDir); err != nil {
